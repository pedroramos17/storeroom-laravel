    ...
    <x-slot name="header">
      <div class="flex justify-between">
        ...
        <form action="{{ route('products.search') }}" method="GET">
            <div class="space-x-6">
                <input type="text" placeholder="{{ __("Search products...") }}" name="query" class="textfield relative py-4 placeholder:text-zinc-300" value="{{ request('query') }}" aria-label="Search" autofocus />
                <x-button type="submit">{{ __("Search") }}</x-button>
                <x-button type="reset"><a href="{{ url('/products/search') }}">{{ __("Clear") }}</a></x-button>
            </div>
        </form>
      </div>
    </x-slot>
          ...
          <div class="flex justify-between mt-4">
            <div class="space-y-4 mx-4">
              <div class="title-card-parent mx-0">
                <h1 class="text-white font-regular text-3xl">Resultados da pesquisa @if($query)para "{{$query}}" @endif</h1>
              </div>
              @if($products->isEmpty() && isset($query))
                  <p class="text-white font-meduim">{{ __('No products found.') }}</p>
              @else
                <div class="pt-4"></div>
                @endif

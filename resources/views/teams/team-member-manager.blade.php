<div>
        ...
        <div class="mt-10 sm:mt-0">
            <x-form-section submit="addTeamMember">
                ...
                <x-slot name="description">
                    {{ __('Adicionar um novo membro em sua equipe, permitirá ele colaborar com você.') }}
                </x-slot>

                <x-slot name="form">
                    <div class="col-span-6">
                        <div class="max-w-xl text-sm text-gray-600 dark:text-gray-400">
                            {{ __('Por favor providencie o email da pessoa que você  gostaria de adicionar nesta equipe.') }}
                        </div>
                    </div>
                    ...
                    @if (count($this->roles) > 0)
                        <div class="col-span-6 lg:col-span-4">
                            <x-label for="role" value="{{ __('Pemissões') }}" />
    ...
    @if ($team->teamInvitations->isNotEmpty() && Gate::check('addTeamMember', $team))
        ...
        <div class="mt-10 sm:mt-0">
            <x-action-section>
                <x-slot name="title">
                    {{ __('Convite de equipe pendente.') }}
                </x-slot>

                <x-slot name="description">
                    {{ __('Essas pessoas foram convidadas para a sua equipe e receberam um e-mail de convite. Elas podem participar da equipe aceitando o convite por e-mail.') }}
                </x-slot>
    ...
    <x-confirmation-modal wire:model="confirmingLeavingTeam">
        ...
        <x-slot name="content">
            {{ __('Você está certo que você gostaria de sair desta equipe?') }}
        </x-slot>
    ...
    <x-confirmation-modal wire:model="confirmingTeamMemberRemoval">
        ...
        <x-slot name="content">
            {{ __('Você está certo que gostaria de remover esta pessoa da sua equipe?') }}
        </x-slot>
